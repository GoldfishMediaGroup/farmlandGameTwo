(function () {
  ...
  function createModal(conf) {
    ...
    conf.steps.forEach((step, index) => {
      ...
      // Принудительная предзагрузка для Сафари
      video.load();
  ...
  function playVideo(videoId) {
    ...
    allVideos.forEach(v => {
        ...
        v.ontimeupdate = null; // Чистим старые события
    });
  ...
  function addResponsiveStyles() {
    ...
    style.textContent = `
      ...
      .v-modal__modal { background: #000; border-radius: 12px; overflow: hidden; position: relative; transform: translateY(30px); transition: transform 0.3s ease; max-width: 80vw; aspect-ratio: 16/9; width: 100%; box-shadow: 0 10px 30px rgba(0,0,0,0.5); }
      ...
      .v-quiz__btn { position: absolute; inset: 0; background: transparent; border: 5px solid red; cursor: pointer; -webkit-tap-highlight-color: transparent; }
